...
def f():
    global zar, voyt, akaynt, temp, feels_like, wind_dir, wind_speed, pressure_mm, humidity, daytime, condition, gorod, lens
    if request.method == 'GET':
        ...
        if 'search' in request.form:
            ...
            try:
                ...
                gorod.insert(0, kop)
                print(temp, feels_like, wind_dir, wind_speed, pressure_mm, humidity, daytime, condition)
                if len(temp) > 6:
                    temp = temp[:6]
                ...
                return render_template('osnovpoisk1.html', m=zar, n=voyt, temp=temp, feels_like=feels_like,
                                       ...
                                       daytime=daytime,
                                       condition=condition, city=gorod, dl=lens, probl='')
            except Exception:
                return render_template('osnovpoisk1.html', m=zar, n=voyt, temp=temp, feels_like=feels_like,
                                       wind_dir=wind_dir,
                                       wind_speed=wind_speed, pressure_mm=pressure_mm, humidity=humidity,
                                       ...
                                       probl='Не удалось найти попробуйте еще раз')
        else:
            return render_template('osnov1.html', m=zar, n=voyt, pr='')
